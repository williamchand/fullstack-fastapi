...
func NewWeddingServer(svc *services.WeddingService) salonappv1.WeddingServiceServer { return &weddingServer{svc: svc} }
...
func (s *weddingServer) CreateWedding(ctx context.Context, req *salonappv1.CreateWeddingRequest) (*salonappv1.WeddingResponse, error) {
    ...
    if req.ConfigJson != "" { _ = json.Unmarshal([]byte(req.ConfigJson), &cfg) }
    var tmplID *uuid.UUID
    if req.TemplateId != "" { id := uuid.MustParse(req.TemplateId); tmplID = &id }
...
func (s *weddingServer) ListGuests(ctx context.Context, req *salonappv1.ListGuestsRequest) (*salonappv1.GuestsResponse, error) {
    ...
    for _, g := range rows { res.Guests = append(res.Guests, toGuestProto(g)) }
    return res, nil
}

func toWeddingProto(w *entities.Wedding) *salonappv1.Wedding {
    var tmplID, payID, domain, slug string
    if w.TemplateID != nil { tmplID = w.TemplateID.String() }
    if w.PaymentID != nil { payID = w.PaymentID.String() }
    if w.CustomDomain != nil { domain = *w.CustomDomain }
    if w.Slug != nil { slug = *w.Slug }
...
func toGuestProto(g *entities.Guest) *salonappv1.Guest {
    var msg string
    if g.Message != nil { msg = *g.Message }
