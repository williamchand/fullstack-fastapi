...
func (r *guestRepository) Add(ctx context.Context, g *entities.Guest) (*entities.Guest, error) {
	out, err := r.queries.AddGuest(ctx, dbgen.AddGuestParams{WeddingID: g.WeddingID, Name: g.Name, Contact: g.Contact, RsvpStatus: dbgen.RsvpStatus(g.RSVPStatus), Message: toPgText(g.Message)})
	if err != nil {
		return nil, err
	}
...
func (r *guestRepository) Update(ctx context.Context, g *entities.Guest) (*entities.Guest, error) {
	out, err := r.queries.UpdateGuest(ctx, dbgen.UpdateGuestParams{ID: g.ID, Name: g.Name, Contact: g.Contact, RsvpStatus: dbgen.RsvpStatus(g.RSVPStatus), Message: toPgText(g.Message)})
	if err != nil {
		return nil, err
	}
	return r.toEntity(out), nil
