...
func (r *weddingRepository) SetTemplate(ctx context.Context, id uuid.UUID, templateID uuid.UUID) (*entities.Wedding, error) {
    out, err := r.queries.SetWeddingTemplate(ctx, dbgen.SetWeddingTemplateParams{ID: id, TemplateID: templateID})
    if err != nil { return nil, err }
    return r.toEntity(out), nil
}

func (r *weddingRepository) SetPayment(ctx context.Context, id uuid.UUID, paymentID uuid.UUID) (*entities.Wedding, error) {
    out, err := r.queries.SetWeddingPayment(ctx, dbgen.SetWeddingPaymentParams{ID: id, PaymentID: paymentID})
    if err != nil { return nil, err }
    return r.toEntity(out), nil
}
...
func (r *weddingRepository) toEntity(v dbgen.Wedding) *entities.Wedding {
    ...
    if v.TemplateID.Valid { id := uuid.UUID(v.TemplateID.Bytes); tmplID = &id }
    var payID *uuid.UUID
    if v.PaymentID.Valid { id := uuid.UUID(v.PaymentID.Bytes); payID = &id }
